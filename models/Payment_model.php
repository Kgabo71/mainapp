...
class Payment_model extends CI_Model {
    ...
    public function check_stripe_payment($identifier = "") {
      ...
      public function check_razorpay_payment($identifier = ""){
        // Start common code of all payment gateways
        $payment_details = $this->session->userdata('payment_details');
        $payment_gateway = $this->db->get_where('payment_gateways', ['identifier' => $identifier])->row_array();
    
        // Load necessary models or classes as required
    
        // Common code for obtaining keys
        $keys = json_decode($payment_gateway['keys'], true);
        $test_mode = $payment_gateway['enabled_test_mode'];
        // End common code
    
        if ($test_mode == 1) {
            $merchant_id = $keys['test_merchant_id'];
            $merchant_key = $keys['test_merchant_key'];
            $passphrase = $keys['test_passphrase'];
        } else {
            $merchant_id = $keys['live_merchant_id'];
            $merchant_key = $keys['live_merchant_key'];
            $passphrase = $keys['live_passphrase'];
        }
        ...
        try {
            ...
            // Remove the signature field from the data for signature calculation
            unset($data['signature']);
    
            // Construct the signature string by concatenating the fields
            $signature_string = '';
            ...
            // Generate the expected signature using your PayFast merchant key and passphrase
            $expected_signature = md5($signature_string . $merchant_key);
    
            if ($expected_signature !== $signature) {
                $error = 'PayFast_Error : Signature verification failed';
                return false;
            }
        } catch (Exception $e) {
            $error = 'PayFast_Error : ' . $e->getMessage();
            return false;
        }
    
        // Verify payment details with your system or database
        // For example:
        // $response = $this->verify_payment_with_database($data['m_payment_id']);
    
        // You can perform additional actions based on the payment status or data
        // For example:
        // if ($response['status'] === 'COMPLETE') {
        //    // Update payment status in your system
        //    $this->update_payment_status($data['m_payment_id'], 'completed');
        // }
    ...
    public function razorpayPrepareData($identifier = ""){
      ...
      if($test_mode == 1){
        ...
        $razorpayOrder = $api->order->create(array(
          'receipt'         => rand(),
          'amount'          => $_SESSION['payable_amount'], // 2000 rupees in paise
    ...
    public function get_transaction_by_payment_id($payment_id) {
        // Implement your database query here to fetch transaction data
        // Example query:
        $query = $this->db->get_where('payment_records', array('payment_id' => $payment_id));
        
        // Check if a row was found
        if ($query->num_rows() > 0) {
            return $query->row_array();
        } else {
            return false; // Return false if no transaction data found
        }
    }
}
